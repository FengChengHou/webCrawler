...
/**
 * Miscellaneous utilities methods for web applications.
 *
 * @author Rod Johnson
...
public abstract class WebUtils {
     ...
     * in <code>web.xml</code>): "defaultHtmlEscape".
     */
    public static final String HTML_ESCAPE_CONTEXT_PARAM = "defaultHtmlEscape";

    /**
     * Web app root key parameter at the servlet context level (i.e. a
     * context-param in <code>web.xml</code>): "webAppRootKey".
     */
     ...
     * in <code>web.xml</code>. Default is "webapp.root".
     * <p/>
     * Can be used for tools that support substition with
     * <code>System.getProperty</code> values, like log4j's "${key}" syntax
     * within log file locations.
     *
     * @param servletContext the servlet context of the web application
     * @throws IllegalStateException if the system property is already set, or
     *                               if the WAR file is not expanded
     ...
     * <code>web.xml</code> (if any). Falls back to <code>false</code> in case
     ...
     * @throws FileNotFoundException if the path cannot be resolved to a
     *                               resource
     ...
     * @throws IllegalArgumentException if the session attribute could not be
     *                                  instantiated
     */
     ...
     * that the HttpSessionMutexListener needs to be defined in
     * <code>web.xml</code>. Falls back to the HttpSession itself if no mutex
     * attribute found.
     * <p/>
     ...
     * resolution. <code>javax.servlet.error.status_code</code>,
        ...
        while (it.hasNext()) {
            ...
            if (!(entry.getKey() instanceof String)) {
                throw new IllegalArgumentException("Invalid key [" + entry.getKey()
                        + "] in attributes Map - only Strings allowed as attribute keys");
            }
     ...
     * @author xfzhou
     */
    public static String getCookieValue(HttpServletRequest request, String name) throws IOException {
    ...
    public static void createCookie(HttpServletResponse response, String key, String value, int maxAge) {
        ...
        value = new String(Base64Util.encode(value.getBytes()));
    ...
    public static void createCookie(HttpServletResponse response, String key, String value) {
        createCookie(response, key, value, 604800);
    }
     ...
     * @param prefix  the beginning of parameter names (if this is null or the
     *                empty string, all parameters will match)
     ...
     *
     * @param request     current servlet request
     * @param paramPrefix the parameter prefix to check for (e.g. "_target" for
     *                    parameters like "_target1" or "_target2")
     * @param currentPage the current page, to be returned as fallback if no
